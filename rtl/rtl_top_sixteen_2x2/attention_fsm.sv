...
//
// N_ARRAY: systolic array side length (N/4 = 2 for sixteen-2x2 variant).
// COMP_DONE = 2*N_ARRAY + N - 3   (last compute cycle index, = 9 for N_ARRAY=2, N=8)
// FEED_LAST = N_ARRAY + N - 2     (last feed cycle index,    = 8 for N_ARRAY=2, N=8)
//
...
module attention_fsm #(
    ...
    parameter int N_ARRAY    = 2    // systolic array side; timing derived from this
)(
    input  logic       clk,
    input  logic       rst_n,
    input  logic       start,
    ...
    // Last compute cycle: PE[N_ARRAY-1][N_ARRAY-1] accumulates k=N-1
    //   at t = 2*(N_ARRAY-1) + (N-1)
    localparam int COMP_DONE = 2*N_ARRAY + N - 3;  // = 9 for N_ARRAY=2, N=8
    // Last feed cycle: row/col N_ARRAY-1 needs k=N-1 at t = (N_ARRAY-1)+(N-1)
    localparam int FEED_LAST = N_ARRAY + N - 2;    // = 8 for N_ARRAY=2, N=8
