module controller_fsm (
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!data_sel) begin
                if (a_count < 5'd16) begin
                    a_count <= a_count + 5'd1;
                end
            end
            else begin
                if (b_count < 5'd16) begin
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (compute_count < 4'd9) begin
                compute_count <= compute_count + 4'd1;
            end
        end
    end
    ...
    always_comb begin
        active_count = data_sel ? b_count[3:0] : a_count[3:0];
        accept_input = 1'b0;
        load_en     = 1'b0;
        load_row    = active_count[3:2];
        load_col    = active_count[1:0];
        data_ready  = 1'b0;
        cycle_count = compute_count[2:0];
        feed_en     = 1'b0;
        clear_acc   = 1'b0;
        compute_en  = 1'b0;
        compute_done = 1'b0;
        ...
        case (state)
            ...
            LOAD: begin
                data_ready = data_sel ? (b_count < 5'd16) : (a_count < 5'd16);
                accept_input = data_valid && data_ready;
                load_en = accept_input;
                if ((a_count == 5'd16) && (b_count == 5'd16)) begin
                    next_state = CLEAR;
                end
            end
